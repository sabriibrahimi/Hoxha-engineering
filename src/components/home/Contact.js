...
const Contact = () => {
  ...
  return (
    <section id="contact" className="bg-secondary-dark text-white overflow-hidden section-divider-dark">
      <div className="container-premium">
        <div className="grid lg:grid-cols-12">
          <Reveal className="lg:col-span-7 py-16 sm:py-20 lg:py-28 lg:pr-16">
            <p className="text-[10px] uppercase tracking-[0.26em] text-bronze font-semibold mb-6">{t('contactPage.getInTouch')}</p>
            <h2 className="text-display-sm font-heading max-w-3xl text-balance mb-7">{t('cta.discussTitle')}</h2>
            <p className="text-white/75 leading-relaxed max-w-2xl mb-9">{t('cta.discussDescription')}</p>
            <Link to={path} className="btn btn-primary px-8">{t('contactPage.sendMessage')} <span aria-hidden="true">&rarr;</span></Link>
          </Reveal>
          ...
          <Reveal delay={0.1} className="lg:col-span-5 bg-secondary-dark lg:-mr-[50vw] lg:pr-[50vw]">
            <div className="h-full py-14 sm:py-16 lg:py-28 lg:pl-12 xl:pl-16">
              ...
              <p className="text-xs text-white/60 mt-6">{t('contact.monday')}<br />{t('contact.saturday')}</p>
            </div>
          </Reveal>
        </div>
      </div>
...
const Detail = ({ label, value, href }) => (
  <div className="border-l border-white/15 pl-5">
    <p className="text-[9px] uppercase tracking-[0.2em] text-white/60 mb-2">{label}</p>
